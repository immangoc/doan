...
export async function confirmGateIn(params: GateInParams): Promise<void> {
  ...
  // ── Step 1: Validate container exists and has an approved order ──────────────
  let skipGateIn = false;
  let confirmedContainerId = containerCode;

  if (!params.skipContainerCheck) {
    // Validate container exists in DB (customer must have registered it)
    const checkRes = await apiFetch(`/admin/containers/${encodeURIComponent(containerCode)}`);
    if (checkRes.status === 404) {
      throw new Error(
        `Container "${containerCode}" chưa được đăng ký trong hệ thống. ` +
        `Khách hàng phải đăng ký container, tạo đơn hàng và được admin duyệt trước khi nhập kho.`
      );
    } else if (checkRes.ok) {
    ...
    // Validate container has an approved order (business rule gate)
    if (!skipGateIn) {
      const orderRes = await apiFetch(`/admin/orders/by-container/${encodeURIComponent(confirmedContainerId)}`);
      if (!orderRes.ok) {
        throw new Error(
          `Container "${confirmedContainerId}" chưa có đơn hàng hợp lệ. ` +
          `Khách hàng phải tạo đơn hàng và được admin duyệt trước khi nhập kho.`
        );
      }
      const orderJson = await orderRes.json().catch(() => ({})) as Rec;
      const orderData = (orderJson.data ?? orderJson) as Rec | null;
      if (!orderData) {
        throw new Error(
          `Container "${confirmedContainerId}" chưa có đơn hàng. ` +
          `Vui lòng hoàn tất luồng: Tạo container → Tạo đơn hàng → Admin duyệt → Nhập kho.`
        );
      }
      const orderStatus = String(orderData.statusName ?? orderData.status ?? '').toUpperCase();
      const validStatuses = ['APPROVED', 'WAITING_CHECKIN', 'LATE_CHECKIN'];
      if (!validStatuses.includes(orderStatus)) {
        throw new Error(
          `Đơn hàng #${orderData.orderId ?? ''} của container "${confirmedContainerId}" chưa được duyệt ` +
          `(trạng thái hiện tại: ${orderStatus}). ` +
          `Chỉ nhập kho sau khi đơn hàng được APPROVED.`
        );
      }
    }
  }
