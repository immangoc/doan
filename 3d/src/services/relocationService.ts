...
export interface RelocateParams {
  containerId: string; // backend container_id (code string)
...
export async function relocateContainer(params: RelocateParams): Promise<void> {
  // Backend expects RelocationRequest:
  // { containerId, targetSlotId, targetTier }
  if (params.slotId == null) {
    throw new Error('Thiếu slotId để dời container');
  }
  const body: Rec = {
    containerId: params.containerId,
    targetSlotId: params.slotId,
